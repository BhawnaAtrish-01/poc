...
class ColDeletionApprovedView(APIView):
    def delete(self, request, record_id, *args, **kwargs):
        ...
        http://localhost:8000/api/col_deletion_approval/66b9fb790b2700bfd39597ba/
        """
        try:
...
class ColDeletionRejectedView(APIView):
    def delete(self, request, record_id, *args, **kwargs):
        """
        Soft deleting the fields approved by admin.
        http://localhost:8000/api/col_deletion_rejection/66b9fb790b2700bfd39597ba/
        """
        try:
            result = deleted_columns.update_one(
                {"_id": ObjectId(record_id)}, {"$unset": {"is_deleted": ""}}
            )
...
class RecordDeletionDisapproved(APIView):
    def post(self, request, record_id, *args, **kwargs):
        """
        Remove the 'is_deleted' field from a specific row.
        """
        try:
            result = table_data.update_one(
                {"_id": ObjectId(record_id)}, {"$unset": {"is_deleted": ""}}
            ...
            return Response(
                {"message": "'is_deleted' field removed successfully"},
                status=status.HTTP_200_OK,
            )

        except Exception as e:
            return Response(
                {"error": f"Error removing 'is_deleted' field in MongoDB: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
